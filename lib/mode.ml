...
let tool_category tool_name =
  ...
  | "masc_listen" | "masc_who" | "masc_reset" -> Comm
...
let category_description = function
  ...
  | Health -> "Maintenance: heartbeat, gc, cleanup_zombies"
  | Discovery -> "Agent discovery: register_capabilities, find_by_capability"
  ...
  | Auth -> "Authentication: auth_enable, auth_create_token"
