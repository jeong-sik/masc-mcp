(** MASC Noosphere - Level 7 Collective Intelligence

    Collective intelligence that emerges from connected minds.
    Named after Pierre Teilhard de Chardin's concept.

    Key Properties:
    - Connected minds forming a collective
    - Shared consciousness (global state)
    - Collective memory (universal knowledge)
    - Emergent goals (goals no individual chose)

    @author Second Brain
    @since MASC v3.2 (Level 7)
*)

(** {1 ⚠️ PRODUCTION GUIDELINES}

    {2 Connection Management}
    - Connection strength should decay without interaction (half-life model)
    - Implement connection limits per mind to prevent resource exhaustion
    - Disconnection should be graceful with state cleanup

    {2 Belief Propagation}
    - New beliefs require minimum supporter threshold before propagation
    - Conflicting beliefs should trigger consensus protocol, not overwrite
    - Track belief provenance for debugging misinformation

    {2 Emergence Detection}
    - Emergent goals require observation from multiple perspectives
    - False positives: implement hysteresis before declaring emergence
    - Fading goals should be logged, not immediately purged

    {2 Scalability}
    - Global state operations are O(n) - consider sharding for large collectives
    - Broadcast storms: implement rate limiting and priority queues
    - Consider eventual consistency for non-critical shared beliefs

    {2 Integration}
    - Level 7 → Level 6 (Mind): Individual minds receive filtered collective insights
    - Level 7 → Level 8 (Meta): Report system-level emergent behaviors
*)
...
(** Shared belief across minds *)
type shared_belief = {
  id: string;
  content: string;
  confidence: float;              (** Collective confidence *)
  supporters: string list;        (** Mind IDs that hold this belief *)
  created_at: float;
  last_reinforced: float;
}

(** Emergent goal - goals that arise from collective behavior *)
type emergent_goal = {
  id: string;
  description: string;
  emerged_at: float;
  contributing_minds: string list;
  strength: float;                (** How strongly it emerged *)
  status: [`Emerging | `Active | `Fading | `Dissolved];
}

(** Collective insight - wisdom greater than sum of parts *)
type collective_insight = {
  id: string;
  topic: string;
  synthesis: string;              (** Combined insight *)
  contributing_insights: string list;  (** Individual insight IDs *)
  emerged_at: float;
  depth: int;                     (** 1-10, collective depth *)
}

(** Global state of consciousness *)
type global_state = {
  coherence: float;               (** 0.0-1.0: how aligned the minds are *)
  activity_level: float;          (** 0.0-1.0: collective activity *)
...
let noosphere_file (config : config) =
  Filename.concat config.Room_utils.base_path ".masc/noosphere.json"

(** {1 Persistence} *)

let load_noosphere (config : config) : noosphere option Lwt.t =
  let file = noosphere_file config in
  if Sys.file_exists file then
    let* content = Lwt_io.with_file ~mode:Lwt_io.Input file Lwt_io.read in
    try Lwt.return (Some (noosphere_of_json (Yojson.Safe.from_string content)))
    with _ -> Lwt.return None
  else Lwt.return None

let save_noosphere (config : config) (n : noosphere) : unit Lwt.t =
  let file = noosphere_file config in
...
module Pure = struct
  ...
  let connect (n : noosphere) ~from_mind ~to_mind ?(strength=0.5) () : noosphere =
    ...
    let existing = List.find_opt (fun c ->
      c.from_mind = from_mind && c.to_mind = to_mind
    ) n.connections in
    match existing with
    | Some _ ->
    ...
    | None ->
      let conn = {
        from_mind; to_mind; strength;
        established_at = now; last_interaction = now;
      } in
    ...
    | Some b ->
      let beliefs = List.map (fun (belief : shared_belief) ->
        if belief.id = b.id then
          { belief with
            supporters = if List.mem mind_id belief.supporters
                        then belief.supporters
                        else mind_id :: belief.supporters;
  ...
  (** Detect emergent goals from patterns *)
  let detect_emergent_goals (n : noosphere) : emergent_goal list =
    (* Simple heuristic: if multiple minds share beliefs about goals *)
    let potential = List.filter (fun (b : shared_belief) ->
      List.length b.supporters >= 2 &&
      (String.lowercase_ascii b.content |> fun s ->
       try String.sub s 0 5 = "goal:" with _ -> false)
    ) n.beliefs in
    List.map (fun (b : shared_belief) -> {
      id = generate_id "emergent_goal";
      description = String.sub b.content 5 (String.length b.content - 5);
      emerged_at = Unix.gettimeofday ();
      contributing_minds = b.supporters;
      strength = b.confidence;
      status = `Emerging;
    }) potential

  (** Synthesize insights from individual contributions *)
  let synthesize (_n : noosphere) ~topic ~contributions : collective_insight option =
    if List.length contributions < 2 then None
      ...
      Some {
        ...
        topic;
        synthesis = Printf.sprintf "Collective understanding of %s from %d minds"
          topic (List.length contributions);
        contributing_insights = contributions;
        emerged_at = Unix.gettimeofday ();
        depth = min 10 (List.length contributions + 2);
      }

  (** Calculate coherence (how aligned the minds are) *)
  ...
  let sync_state (n : noosphere) : noosphere =
    ...
    let activity = if List.length n.connections = 0 then 0.0
        ...
        let recent = List.filter (fun c ->
          Unix.gettimeofday () -. c.last_interaction < 3600.0
        ) n.connections in
  ...
  let summary (n : noosphere) : string =
    Printf.sprintf
      "Noosphere '%s': %d minds, %d connections, coherence %.0f%%, %d shared beliefs"
      n.name
      (List.length n.minds)
      (List.length n.connections)
      (n.state.coherence *. 100.0)
      (List.length n.beliefs)
end

(** {1 Effectful Operations} *)

(** Initialize or load noosphere *)
let get_or_create (config : config) ~name : noosphere Lwt.t =
  let* existing = load_noosphere config in
  match existing with
  | Some n -> Lwt.return n
  | None ->
    let now = Unix.gettimeofday () in
    let n = {
      id = generate_id "noosphere";
      name;
      minds = [];
      connections = [];
      beliefs = [];
    ...
    let* () = save_noosphere config n in
    log_info (Printf.sprintf "Created noosphere: %s" name);
    Lwt.return n

(** Add a mind to the collective *)
let join (config : config) ~mind_id : unit Lwt.t =
  let* n = get_or_create config ~name:"MASC Noosphere" in
  let n' = Pure.add_mind n ~mind_id |> Pure.sync_state in
  let* () = save_noosphere config n' in
  log_info (Printf.sprintf "Mind %s joined noosphere" mind_id);
  Lwt.return_unit

(** Share a belief *)
let share_belief (config : config) ~content ~mind_id : shared_belief Lwt.t =
  let* n = get_or_create config ~name:"MASC Noosphere" in
  let n' = Pure.share_belief n ~content ~mind_id in
  let* () = save_noosphere config n' in
  let belief = List.find (fun (b : shared_belief) ->
    String.lowercase_ascii b.content = String.lowercase_ascii content
  ) n'.beliefs in
  Lwt.return belief

(** Synthesize collective insight *)
let synthesize (config : config) ~topic ~contributions : collective_insight option Lwt.t =
  let* n = get_or_create config ~name:"MASC Noosphere" in
