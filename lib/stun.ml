...
let decode_message_type type_val =
  ...
  let msg_method = match method_bits with
    | 0x0001 -> Binding
    | _ -> Binding  (* Only Binding for now *)
  in
    ...
    if Bytes.length data < off + 4 + padded_len then
      ...
      let value = match attr_type with
        ...
        | ERROR_CODE ->
          ...
          let reason = Bytes.sub_string value_data 4 (value_len - 4) in
          Error_code { code; reason }
...
let verify_integrity msg ~key =
  ...
  let rec split_at_integrity acc = function
    | [] -> None, List.rev acc
    | { attr_type = MESSAGE_INTEGRITY; value = Message_integrity b; _ } :: _ ->
      Some b, List.rev acc
    | attr :: rest -> split_at_integrity (attr :: acc) rest
  in
  match split_at_integrity [] msg.attributes with
  | None, _ -> false
  | Some expected, attrs_before ->
    (* Create message with only attributes before MESSAGE-INTEGRITY *)
    let msg_without_mi = { msg with attributes = attrs_before } in
    let calculated = calculate_integrity msg_without_mi ~key in
...
(* ============================================
   Eio Client Functions (OCaml 5.x Direct-Style)
   ============================================ *)

(** Send binding request using Eio (OCaml 5.x direct-style async).

    This is the recommended API for OCaml 5.x applications.
    Uses Eio.Net for non-blocking I/O without monadic chaining.

    @param net Eio network capability
    @param sw Eio switch for resource management
    @param server STUN server address (host:port or just host)
    @param timeout Timeout in seconds (default: 3.0)
    @return Binding result or error
*)
let binding_request_eio ~net ~sw ~server ?timeout () =
  ...
  begin
    try
      Eio.Fiber.first
        (fun () ->
          let _from, got = Eio.Net.recv sock response_buf in
          let response_bytes = Cstruct.to_bytes (Cstruct.sub response_buf 0 got) in
  ...
  | Error e -> Error e
  | Ok response_bytes ->
    match decode response_bytes with
    | Error e -> Error e
    | Ok response ->
      (* Verify transaction ID *)
      if not (Bytes.equal response.transaction_id request.transaction_id) then
...
(** Convenience wrapper to run Eio binding request.
    ...
    @return Binding result or error
*)
let binding_request_eio_run ~server ?timeout () =
  Eio_main.run @@ fun env ->
  Eio.Switch.run @@ fun sw ->
  let net = Eio.Stdenv.net env in
  binding_request_eio ~net ~sw ~server ?timeout ()
