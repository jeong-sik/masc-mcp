...
(** {1 Eio Client Functions (OCaml 5.x)} *)
...
(** Send binding request using Eio (OCaml 5.x direct-style async).
    ...
    {[
      ...
      match Stun.binding_request_eio ~net ~sw ~server:"stun.l.google.com" () with
      | Ok result -> Printf.printf "Public IP: %s:%d\n" result.mapped_address.ip result.mapped_address.port
...
(** Convenience wrapper to run Eio binding request.
    ...
    Example:
    {[
      match Stun.binding_request_eio_run ~server:"stun.l.google.com" () with
      | Ok result -> Printf.printf "Public IP: %s\n" result.mapped_address.ip
      | Error e -> Printf.eprintf "Error: %s\n" e
    ]}
