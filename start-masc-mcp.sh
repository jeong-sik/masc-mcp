...
# Usage: ./start-masc-mcp.sh [--stdio] [--http] [--eio] [--port PORT] [--base-path PATH|--path PATH]
...
if [ -z "$MASC_EXE" ]; then
    ...
    if curl -fsSL -o "$RELEASE_BINARY" "$RELEASE_URL" 2>/dev/null; then
        ...
        dune build --root "$SCRIPT_DIR" bin/main.exe 1>&2
        if [ -x "$LOCAL_EXE" ]; then
            MASC_EXE="$LOCAL_EXE"
        else
...
if [ "$EIO_MODE" = "true" ]; then
    ...
    echo "  Base path (resolved): $RESOLVED_BASE_PATH" >&2
    ...
    echo "  MCP endpoint: http://127.0.0.1:$PORT/mcp" >&2
    echo "  SSE: use Accept: text/event-stream on /mcp" >&2
    exec "$SELECTED_EXE" --http --port "$PORT" --base-path "$BASE_PATH"
else
