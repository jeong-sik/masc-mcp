...
let with_test_config name f =
  ...
  let tmp_dir = Filename.concat (Filename.get_temp_dir_name ())
    (Printf.sprintf "ralph_eio_%s_%d_%d" name (Unix.getpid ())
...
(** Test: 3 agents running Ralph simultaneously (Phase 1 feature) *)
let test_eio_multi_agent_ralph () =
  with_test_config "multi_agent" @@ fun _env _fs config ->
  (* 3 agents get their own independent Ralph states *)
  ...
  (* Each agent can set running=true independently (simulating ralph_loop start) *)
...
let eio_tests = [
  ...
  "multi-agent Ralph", `Quick, test_eio_multi_agent_ralph;
  "list ralph states", `Quick, test_eio_list_walph_states;
]
